...
class googlemerchant extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->DB_PREFIXversions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);
    ...
    public function generateFeed()
    {
        ...
        /* Adding availability, image_link, and price fields */

<g:availability>' . ($product['quantity'] > 0 ? 'in stock' : 'out of stock') . '</g:availability>

<g:image_link>' . Tools::getShopDomainSsl(true) . _PS_IMG_ . 'p/' . Image::getImgFolderStatic($product['id_image']) . $product['id_image'] . '.jpg</g:image_link>

<g:price>' . number_format($product['price'], 2) . ' ZAR</g:price>
foreach ($products as $product) {
    ...
    private function getAdditionalImages($id_product)
    {
        ...
        foreach ($result as $row) {
            $image_link = $this->context->link->getImageLink(null, $row['id_image']);
            if ($image_link) {
                $images[] = $image_link;
            }
        }
