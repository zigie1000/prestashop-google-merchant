...
class googlemerchant extends Module
{
    ...
    public function generateFeed()
    {
        ...
        foreach ($products as $product) {
            ...
            $item->addChild('g:brand', htmlspecialchars($product['manufacturer_name'] ? $product['manufacturer_name'] : 'Unknown'));
            ...
            foreach ($item->children() as $child) {
                if (empty($child)) {
                    $this->logError('Empty field: ' . $child->getName() . ' for product ID: ' . $product['id_product']);
                    unset($item->{$child->getName()});
                }
            }
        }

        // Save XML file
        file_put_contents($this->feedFile, $xml->asXML());
    }
