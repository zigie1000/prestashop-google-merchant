...
class googlemerchant extends Module
{
    ...
    public function generateFeed()
    {
        ...
        foreach ($products as $product) {
            ...
            // Ensure image link is correctly generated
            $imageLink = $this->context->link->getImageLink($product['link_rewrite'], $product['id_image']);
            $item->addChild('g:image_link', htmlspecialchars($imageLink));
            ...
            // Handle EAN (GTIN)
            $gtin = !empty($product['ean13']) ? htmlspecialchars($product['ean13']) : '';
            $item->addChild('g:gtin', $gtin);

            $item->addChild('g:mpn', htmlspecialchars($product['id_product']));
            $item->addChild('g:condition', 'new');

            // Product type and Google product category mapping
            $productType = htmlspecialchars($product['category_name']) ?? '';
            $googleProductCategory = isset($product['google_product_category']) ? htmlspecialchars($product['google_product_category']) : '';
            $item->addChild('g:product_type', $productType);
            $item->addChild('g:google_product_category', $googleProductCategory);
